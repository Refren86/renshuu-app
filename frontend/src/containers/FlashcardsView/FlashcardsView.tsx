...
function FlashcardsView() {
  ...
  const handleClick = useCallback(async (word: TFlashcard, newStatus: FlashcardStatus) => {
      ...
      <section className="flex flex-col justify-center items-center h-[calc(100dvh-49px)] overflow-hidden">
        <div className="relative w-full max-w-md h-96">
          <AnimatePresence onExitComplete={() => setIsAnimating(false)}>
            ...
            >
              {currentFlashcard && (
                <Flashcard flashcard={currentFlashcard} isAnimating={isAnimating} onCopy={handleCopyWord} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
